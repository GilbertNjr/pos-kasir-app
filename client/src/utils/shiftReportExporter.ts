...
// 3. PRINT TO PDF - FORMAT TABEL A4 GRID HIJAU (FORMAT RESMI LAPORAN REKAPITULASI SHIFT GAMBAR #2)
export const printShiftA4PDF = (options: ShiftReportExportOptions) => {
  const {
    storeName = 'Kedai POS',
    dateStr = new Date().toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }),
    shiftId = 'Shift Sesi Aktif',
    ...
    itemList.length > 0
      ? itemList
          .map(
            (item) => `
        <tr>
          <td style="text-align: center; border: 1px solid #cbd5e1; padding: 6px;">${item.qty}</td>
          <td style="text-align: left; border: 1px solid #cbd5e1; padding: 6px;">${item.name}</td>
          <td style="text-align: right; border: 1px solid #cbd5e1; padding: 6px;">${formatRupiah(item.price)}</td>
          <td style="text-align: right; border: 1px solid #cbd5e1; padding: 6px; font-weight: bold;">${formatRupiah(item.subtotal)}</td>
        </tr>
      `
          )
          .join('')
      : `
        <tr>
          <td colspan="4" style="text-align: center; font-style: italic; color: #64748b; padding: 8px; border: 1px solid #cbd5e1;">(Belum ada barang/jasa terjual)</td>
        </tr>
      `;
    ...
    expenseList.length > 0
      ? expenseList
          .map(
            (exp) => `
        <tr>
          <td colspan="3" style="text-align: left; border: 1px solid #cbd5e1; padding: 6px 12px;">- ${exp.description}</td>
          <td style="text-align: right; border: 1px solid #cbd5e1; padding: 6px; color: #dc2626; font-weight: bold;">${formatRupiah(exp.amount)}</td>
        </tr>
      `
          )
          .join('')
      : `
        <tr>
          <td colspan="3" style="text-align: left; font-style: italic; color: #64748b; border: 1px solid #cbd5e1; padding: 6px 12px;">(Tidak ada pengeluaran kas)</td>
          <td style="text-align: right; border: 1px solid #cbd5e1; padding: 6px;">Rp 0</td>
        </tr>
      `;
  ...
  printWindow.document.write(`
    ...
    <html>
    <head>
      <title>Laporan Rekapitulasi Shift - ${dateStr}</title>
      <style>
        @page {
          size: A4 portrait;
          margin: 12mm 15mm;
    ...
    <body>
      ...
      <table class="meta-table">
        <tr>
          <td style="width: 50%;"><strong>Tanggal:</strong> ${formattedDateStr}</td>
          <td style="width: 50%;"><strong>Shift Sesi:</strong> ${cleanShiftId}</td>
        </tr>
        <tr>
          <td><strong>Jam Operasional:</strong> ${shiftDurationStr || 'Sesi Shift Aktif'}</td>
          <td><strong>Pegawai Shift Jaga:</strong> ${dutyUsersStr}</td>
      ...
      <!-- TABEL 1: RINCIAN BARANG/JASA TERJUAL -->
      <table class="data-table">
        <thead>
          <tr>
            <th style="width: 10%;">Qty</th>
            <th style="width: 50%; text-align: left;">Nama Barang / Jasa</th>
            <th style="width: 20%; text-align: right;">Harga Satuan</th>
            <th style="width: 20%; text-align: right;">Total Jumlah</th>
          </tr>
        </thead>
        <tbody>
          ${itemRowsHtml}
          <tr class="bg-subtotal">
            <td colspan="3" style="text-align: right; font-weight: bold; border: 1px solid #cbd5e1;">TOTAL PENJUALAN KOTOR</td>
            <td style="text-align: right; font-weight: bold; color: #047857; border: 1px solid #cbd5e1;">${formatRupiah(totalGrossSales)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="data-table">
        <thead>
          <tr class="bg-expense-head">
            <th colspan="3" style="text-align: left; border: 1px solid #047857;">PENGELUARAN SHIFT</th>
            <th style="text-align: right; border: 1px solid #047857;">NOMINAL</th>
          </tr>
        </thead>
        <tbody>
          ${expenseRowsHtml}
          <tr class="bg-subtotal">
            <td colspan="3" style="text-align: right; font-weight: bold; color: #dc2626; border: 1px solid #cbd5e1;">TOTAL PENGELUARAN SHIFT</td>
            <td style="text-align: right; font-weight: bold; color: #dc2626; border: 1px solid #cbd5e1;">${formatRupiah(totalExpenses)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="total-box-container">
        <div class="total-box">
          <div class="total-box-title">TOTAL SHIFT BERSIH (UANG FISIK KASIR)</div>
          <div class="total-box-amount">${formatRupiah(netTotal)}</div>
        </div>
      </div>
